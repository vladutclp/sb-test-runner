...
EditTask.play = async ({ canvasElement }) => {
  ...
  const heading = await  canvas.findByTestId('heading')
  await expect(heading).toHaveClass('mainClass');
...
// TestTask.parameters = Default.parameters;
// TestTask.play = async ({ canvasElement }) => {
//   const canvas = within(canvasElement);
//   const heading =  canvas.findByRole('heading', 'Taskbox');
